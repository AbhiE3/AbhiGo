...
const Wrapper = ({ token, handleLogout, children }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      {/* Navigation */}
      <nav className="sticky top-0 left-0 right-0 z-20 bg-transparent">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20 text-blue-500">
            <Link to="/" className="flex items-center">
              ...
              <span className="text-xl font-bold text-blue-500">AbhiGo</span>
            </Link>
            <div className="flex items-center space-x-6">
              {token ? (
                <>
                  <Link
                    to="/my-bookings"
                    className="px-5 py-2 text-md font-semibold text-blue-500"
                  >
                    My Bookings
                  </Link>
                  <button
                    onClick={logout}
                    className="p-2 rounded-full text-blue-500 hover:bg-transparent hover:bg-opacity-20 transition-colors"
                    aria-label="Logout"
                  >
                    <LogoutIcon />
                  </button>
